...
static void put(MyTigr_Save* s, unsigned v) {
    fputc(v, s->out);
    s->crc = (s->crc >> 4) ^ crctable[(s->crc & 15) ^ (v & 15)];
    s->crc = (s->crc >> 4) ^ crctable[(s->crc & 15) ^ (v >> 4)];
}

static void updateAdler(MyTigr_Save* s, unsigned v) {
...
static void put32(MyTigr_Save* s, unsigned v) {
    put(s, (v >> 24) & 0xff);
    put(s, (v >> 16) & 0xff);
    put(s, (v >> 8) & 0xff);
    put(s, v & 0xff);
...
void putbits(MyTigr_Save* s, unsigned data, unsigned bitcount) {
    while (bitcount--) {
...
void putbitsr(MyTigr_Save* s, unsigned data, unsigned bitcount) {
    while (bitcount--)
        putbits(s, data >> bitcount, 1);
}

static void begin(MyTigr_Save* s, const char* id, unsigned len) {
    put32(s, len);
    s->crc = 0xffffffff;
    put(s, id[0]);
    put(s, id[1]);
...
static void literal(MyTigr_Save* s, unsigned v) {
...
static void encodelen(MyTigr_Save* s, unsigned code, unsigned bits, unsigned len) {
    literal(s, code + (len >> bits));
    putbits(s, len, bits);
    putbits(s, 0, 5);
}

static void endrun(MyTigr_Save* s) {
    s->runlen--;
    literal(s, s->prev);
...
static void encodeByte(MyTigr_Save* s, unsigned char v) {
    updateAdler(s, v);
...
static void savePngHeader(MyTigr_Save* s, Tigr* bmp) {
    fwrite("\211PNG\r\n\032\n", 8, 1, s->out);
    begin(s, "IHDR", 13);
    put32(s, bmp->w);
    put32(s, bmp->h);
...
static long savePngData(MyTigr_Save* s, Tigr* bmp, long dataPos) {
    int x, y;
    long dataSize;
    begin(s, "IDAT", 0);
