...
#define MYTIGR_CLIP1(X, DW, W) \
    if (X + W > DW)     \
        W = DW - X;
#define MYTIGR_CLIP()                  \
    MYTIGR_CLIP0(dst->cx, dx, sx, w);  \
    MYTIGR_CLIP0(dst->cy, dy, sy, h);  \
    MYTIGR_CLIP0(0, sx, dx, w);        \
    MYTIGR_CLIP0(0, sy, dy, h);        \
    MYTIGR_CLIP1(dx, dst->cx + cw, w); \
    MYTIGR_CLIP1(dy, dst->cy + ch, h); \
    MYTIGR_CLIP1(sx, src->w, w);       \
    MYTIGR_CLIP1(sy, src->h, h);       \
    if (w <= 0 || h <= 0)       \
    return
