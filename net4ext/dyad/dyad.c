...
static int stream_initSocket(
  ...
  if (stream->sockfd == INVALID_SOCKET) {
    stream_error(stream, "could not create socket", netGetError);
    return -1;
  }
...
static void stream_handleReceivedData(dyad_Stream *stream) {
  for (;;) {
    ...
    if (size <= 0) {
      if (size == 0 || netGetError != EWOULDBLOCK) {
        /* Handle disconnect */
        dyad_close(stream);
        return;
      } else {
...
int dyad_listenEx(
  ...
  if (err) {
    stream_error(stream, "could not get addrinfo", netGetError);
    goto fail;
  }
  /* Init socket */
  ...
  if (err) {
    stream_error(stream, "could not bind socket", netGetError);
    goto fail;
  }
  err = listen(stream->sockfd, backlog);
  if (err) {
    stream_error(stream, "socket failed on listen", netGetError);
    goto fail;
  }
